...
const H1 = 'margin: 0; font-size: 20px; font-weight: 800; letter-spacing: -.02em; color: #0f2f2b; text-wrap: pretty;';
...
const HR = 'margin: 2px 0; border: none; border-top: 1px solid #f1eee9;';
...
export function parseBlocks(src) {
  ...
  for (const raw of String(src || '').split('\n')) {
    ...
    // A rule between sections, rather than a stray '---' glued onto the end
    ...
    // The generator emits a top-level '# Title'. Without this it fell through
    // to the paragraph branch and the '#' was printed as literal text.
    const h1 = /^#\s+(.*)$/.exec(line);
    if (h1) { flush(); blocks.push({ type: 'h1', text: h1[1] }); continue; }
...
export default function Markdown({ children }) {
  ...
  return (
    <div style={css('display: flex; flex-direction: column; gap: 14px;')}>
      {blocks.map((b, i) => {
        if (b.type === 'hr') return <hr key={i} style={css(HR)} />;
        if (b.type === 'h1') return <h1 key={i} style={css(H1)}>{inline(b.text)}</h1>;
