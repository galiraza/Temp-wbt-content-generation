  ...
  renderVals() {
    ...
    const pages = this.data.items
      ...
      .map((it, i) => ({
        ...
        slug: it.slug,
        bodyId: 'wc-body-' + i,
        content: st.pageEdits[it.id] !== undefined ? st.pageEdits[it.id] : (it.caption || ''),
      }));
    ...
    return {
      ...
      aiMessages: (st.aiEdit ? st.aiEdit.messages : []).map((m, i) => {
        ...
        return {
          ...
          rowStyle: 'display: flex; flex-direction: column; gap: 6px; align-items: ' + (isUser ? 'flex-end' : 'flex-start') + '; animation: dcFade .24s ease both;',
      ...
      cardVersions: (st.cardHistory ? [0, 1, 2, 3] : []).map(j => {
        ...
        return {
          ...
          thumbLabel: img ? '' : 'TXT',
          style: 'display: flex; align-items: flex-start; gap: 11px; width: 100%; padding: 11px; border-radius: 13px; cursor: pointer; font-family: inherit; transition: border-color .16s ease; border: 1px solid ' + (isAct ? '#f2b98e' : (isCur ? '#c9e5d6' : '#e6e4de')) + '; background: ' + (isAct ? 'linear-gradient(180deg, #fff8f3, #fff2e8)' : '#fff') + ';',
      ...
      runs: this.data.runs.map(r => {
        ...
        return {
          ...
          style: 'display: flex; flex-direction: column; gap: 8px; width: 100%; padding: 13px 14px; border-radius: 14px; cursor: pointer; font-family: inherit; text-align: left; transition: border-color .16s ease; border: 1px solid ' + (sel ? '#f2b98e' : '#e6e4de') + '; background: ' + (sel ? 'linear-gradient(180deg, #fff8f3, #fff2e8)' : '#fff') + ';',
          ...
          {v.formOpen && (
            <div onClick={v.closeForm} style={css('position: fixed; inset: 0; background: rgba(18,16,14,.44); backdrop-filter: blur(3px); display: flex; align-items: center; justify-content: center; padding: clamp(0px, calc(6vw - 34px), 40px) clamp(0px, calc(6vw - 34px), 20px); z-index: 60;')}>
              <div onClick={v.stop} style={css('width: min(100%, 1480px); max-height: 100%; display: flex; flex-direction: column; background: #fff; border-radius: clamp(0px, calc(6vw - 34px), 20px); box-shadow: 0 30px 70px rgba(20,18,16,.3); overflow: hidden; animation: dcPop .18s ease both;')}>
                ...
                <div style={css('flex: 1; overflow: auto; padding: clamp(12px, 2.4vw, 20px) clamp(12px, 2.4vw, 22px) 24px; display: flex; flex-direction: column; gap: 16px; background: #faf9f6;')}>

                  {v.formHasModes && (
                    <div style={css('display: flex; flex-direction: column; gap: 8px; padding: clamp(14px, 2.4vw, 20px); background: #fff; border: 1px solid #e8e5df; border-radius: 16px;')}>
                  ...
                  {v.formCards.map((sec, si) => (
                    <div key={si} style={css('background: #fff; border: 1px solid #e8e5df; border-radius: 16px; overflow: hidden;')}>
        ...
        {v.historyOpen && (
          <div onClick={v.closeHistory} style={css('position: fixed; inset: 0; background: rgba(18,16,14,.44); backdrop-filter: blur(3px); display: flex; align-items: stretch; justify-content: flex-end; z-index: 50;')}>
            <div onClick={v.stop} style={css('width: 100%; max-width: 420px; background: #fff; box-shadow: -20px 0 60px rgba(20,18,16,.24); display: flex; flex-direction: column; animation: dcPop .2s ease both;')}>
              ...
              <div style={css('flex: 1; overflow: auto; padding: 12px; display: flex; flex-direction: column; gap: 8px;')}>
                {v.runs.map((r, ri) => (
